...
class SessionAgentLoader(SafeDataLoader):
    ...
    def batch_load_fn(self, keys: List[Key]) -> Promise:
        ...
        if uncached_keys:
            try:
                for session_agent in SessionAgentModel.batch_get(uncached_keys):
                    key = (session_agent.session_uuid, session_agent.session_agent_uuid)

                    # Cache the result if enabled
                    if self.cache_enabled:
                        self.set_cache_data(key, session_agent)
